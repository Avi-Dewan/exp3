...
from .BigGAN import Generator as BigGen
from .BigGAN import Discriminator as BigDis

class Generator(nn.Module):
    def __init__(self, classes=10, latent_dim=100, img_size=32, channels=3):
        super().__init__()
        self.generator = BigGen(n_classes=classes, dim_z=latent_dim, resolution=img_size)
...
class Discriminator(nn.Module):
    def __init__(self, classes=10, img_size=32, channels=3):
        super().__init__()
        self.discriminator = BigDis(n_classes=classes, resolution=img_size)
